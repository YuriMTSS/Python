...
def tamanho_MB(tamanho_byte):
    tamanho_byte = float(tamanho_byte)
    return (float(tamanho_byte / 1024 ** 2))

def percentual_cada_usuario(lista, total):
...
with open ("usuarios.txt","r") as arquivo:
   ...
   for cadaUsuario in usuarios:
      ...
      valor = tamanho_MB(float(cadaUsuario[2]))
      total += valor
      cadaUsuario.insert((len(cadaUsuario)),valor)
      posicao+=1

   for cadaUsuario in usuarios:
      percentual_cada_usuario(cadaUsuario, total)

media = espaco_medio_ocupado(cadaUsuario,total)
